...
Same as plot_01 but for embedded density-5 subgraphs (exp 2).
Output: local_tests/plot_results/plots/plot_02_fp_bar_embedded5.png
"""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import numpy as np
import matplotlib.pyplot as plt
from parse_logs import (GRAPH_DENSITIES_DEN5, COLOR_DISTRIBUTIONS,
                        load_embedded_fp,
                        run_script_main)
...
def main():
    apply_dark_style()
    data = {algo: load_embedded_fp(algo, 5) for algo in PLOT_ALGOS}

    labels = [f"den{g}\n{c[:3]}" for g in GRAPH_DENSITIES_DEN5
    ...
    ax.set_title("False Positives — Embedded Den-5 Subgraphs (10 graphs × 100 S)",
    ...
    save_fig(fig, os.path.join(OUT_DIR, "plot_02_fp_bar_embedded5.png"))
