...
def main():
    ...
    # Folder that contains G.json and S_1.json ... S_12.json
    INPUT_DIR = os.path.join(BASE_DIR, "example_non_induced")
    ...
    logging.info("Starting example_non_induced run")

    # ---------------- Load and compute G motifs ---------------- #
    ...
    for s_file in s_files:
        total += 1
        s_index = s_file.replace(".json", "")

        S = read_graph_file(os.path.join(INPUT_DIR, s_file))
        ...
        # Preprocess S motifs for non-induced
        ...
        if feasible_sum:
            print(f"✔ {s_file} is found in G")
            logging.info(f"{s_file} FOUND in G")
        else:
            print(f"✘ {s_file} is NOT found in G")
            logging.info(f"{s_file} NOT found in G")
            false_negatives += 1

    print("===================================")
    print("Total S checked:", total)
    print("Misses (should be 0 for true subgraphs):", false_negatives)

    summary_logger.info(
        f"example_non_induced | total={total} misses={false_negatives}"
    )
