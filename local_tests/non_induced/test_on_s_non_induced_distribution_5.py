...
NON_INDUCED_PICKLE_DIR = os.path.join(BASE_DIR, "non_induced", "cache")
INDUCED_PICKLE_DIR = os.path.join(BASE_DIR, "induced", "cache")
LOG_DIR    = os.path.join(BASE_DIR, "non_induced", "logs")

os.makedirs(NON_INDUCED_PICKLE_DIR, exist_ok=True)
os.makedirs(INDUCED_PICKLE_DIR, exist_ok=True)
...
def main():
    SUMMARY_LOG = os.path.join(LOG_DIR, "summary_non_induced_different_distributions_5.log")

    summary_logger = logging.getLogger("summary_non_induced_different_distributions_5")
    ...
    for graph_avg_neighs in [8, 10, 13, 15]:
        for color_distribution in ["uniform", "average", "rare"]:
            ...
            for j in range(10):
                graph_file_name = f"g_den_{graph_avg_neighs}_embedded_den_5_{color_distribution}_{j}"
                ...
                INDUCED_G_PICKLE = os.path.join(INDUCED_PICKLE_DIR, f"{graph_file_name}.pkl")
                NON_INDUCED_G_PICKLE = os.path.join(NON_INDUCED_PICKLE_DIR, f"{graph_file_name}.pkl")

                if os.path.exists(NON_INDUCED_G_PICKLE):
                    with open(NON_INDUCED_G_PICKLE, "rb") as f:
                        g_sum = pickle.load(f)
                    print(f"Loaded cached non induced G motifs for {graph_file_name}")
                else:
                    if os.path.exists(INDUCED_G_PICKLE):
                        with open(INDUCED_G_PICKLE, "rb") as f:
                            g_sum = pickle.load(f).get(MotifsNodeCalculator.MOTIF_SUM_KEY)
