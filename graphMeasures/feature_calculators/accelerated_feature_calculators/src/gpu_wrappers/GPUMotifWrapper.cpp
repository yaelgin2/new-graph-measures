...
py::list GPUMotifCalculatorWrapper(dict converted_dict, std::map<std::string, std::string> configuration, int level, int cudaDevice, bool edges) {
	bool directed = extract<bool>(converted_dict["directed"]);
	ConvertedGNXReciever reciever(converted_dict);
	GPUMotifCalculator calc(configuration, level, directed, cudaDevice, edges);
